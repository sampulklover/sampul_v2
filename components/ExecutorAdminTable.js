...
const ManageExecutor = ({ summary, refreshFunction }) => {
  ...
  const columns = [
    {
      name: <small class="smpl_text-xs-medium">Executor</small>,
      selector: (item) => (
        <td>
    ...
    {
      name: <small class="smpl_text-xs-medium">Name</small>,
      selector: (item) => (
        <td>
          <div class="custom-table-cell">
            <div class="smpl_text-sm-regular crop-text">{item.name}</div>
          </div>
        </td>
      ),
    },
        ...
        <td>
          <div class="custom-table-cell">
            <div
              ...
              onClick={() => downloadData(item, item.executor_code, 'csv')} // Or 'json'
            >
              Download
            </div>
          </div>
  ...
  return (
    <div class="mt-3">
      ...
      <DataTable columns={columns} data={summary.data?.executor} pagination />
    </div>
  );
};

export default ManageExecutor;
